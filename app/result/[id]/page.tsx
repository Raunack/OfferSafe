          ...
          <div className="space-y-3">
            {redditMentions.map((post) => (
              <article key={post.id} className="rounded-lg border border-slate-200 p-4">
                ...
                {(() => {
                  const relevantComments = (redditCommentsByPost[post.id] || [])
                    .filter((comment) =>
                      [
                        "paid",
                        "fee",
                        ...
                        "reported",
                        "police",
                      ].some((keyword) => comment.body.toLowerCase().includes(keyword)),
                    )
                  ...
                  return (
                    <div className="mt-3 space-y-2 rounded bg-slate-50 p-3">
                      <p className="label" style={{ color: "var(--muted)" }}>Top community responses:</p>
                      {relevantComments.map((comment) => (
                        <div key={comment.id} className="rounded border p-2" style={{ borderColor: "var(--card-border)", background: "#fff" }}>
